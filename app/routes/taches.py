...
from app.services.tache_service import create_tache, get_taches_by_user, get_tache, update_tache, delete_tache, toggle_terminated
...
def update_tache_route(tache_id: int, tache_update: TacheUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    ...
    Exemple d'utilisation:
    updated_tache = update_tache_route(tache_id, tache_update, db, current_user)
    """    
    return update_tache(db, tache_id, tache_update, current_user)

@router.delete("/{tache_id}", status_code=204)
def delete_tache_route(tache_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    ...
    Exemple d'utilisation:
    delete_tache_route(tache_id, db, current_user)
    """    
...
def toggle_tache_termination(tache_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    """    
    return toggle_terminated(db, tache_id, current_user)